...
import { Vec2D, degreeToRadians, normalizeAngleInRad } from "src/lib/utils.ts";
...
export class DoorRender extends EntityRender {
    ...
    render(
        ...
        const isDoorOpened = side === 1
            ? doorPositionX >= mapX + 1
            : doorPositionY >= mapY + 1;
        ...
        if (!hitDoorPart) {
            ...
            if (side === 1) {
                ...
                const angToClosestCorner = normalizeAngleInRad(Math.atan2(
                    closestDoorCornetPoint.y - playerPos.y,
                    closestDoorCornetPoint.x - playerPos.x
                ));
                const angleToFaresCorner = normalizeAngleInRad(Math.atan2(
                ...
                if ( rayAngleRad > angToClosestCorner  && rayAngleRad < angleToFaresCorner ) {
                    ...
                    const angle = rayAngleRad < Math.PI
                        ? rayAngleRad
                        : rayAngleRad - Math.PI;
        ...
        const texturePositionX = hitDoorPart === 'frontFace'
            ? Math.floor((texture.width * (rayX + textureOffset.x + rayY + textureOffset.y)) % texture.width)
            : 0;
